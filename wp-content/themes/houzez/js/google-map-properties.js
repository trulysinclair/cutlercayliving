	...
	if (typeof houzez_map_properties !== 'undefined') {
		if ($('#houzez-properties-map').length > 0) {
			...
			var houzezAddMarkers = function (map_properties, houzezMap) {
				...
				var houzezMarkerInfoWindow = function (
					map,
					marker,
					infowindow,
            ...
            let innerHTML = '';
						...
						innerHTML +=
							'<ul class="list-unstyled item-info" style="display: inline-block;">';
						...
						innerHTML += '<ul class="custom-property-attr list-unstyled">';
						// Displays beds in infoWindow
						innerHTML +=
							'<li class="item-type">' +
							'<i class="houzez-icon icon-hotel-double-bed-1 mr-1"></i>' +
							'<span class="hz-figure">' +
							propertyValues.get(`${map_properties[i].property_id}-bed`) +
							'</span>' +
							'</li>';
						// Displays baths in infoWindow
						innerHTML +=
							'<li class="item-type">' +
							'<i class="houzez-icon icon-bathroom-shower-1 mr-1"></i>' +
							'<span class="hz-figure">' +
							propertyValues.get(`${map_properties[i].property_id}-bath`) +
							'</span>' +
							'</li>';
						// Displays sqft in infoWindow
						innerHTML +=
							'<li class="item-type">' +
							'<i class="houzez-icon icon-ruler-triangle mr-1"></i>' +
							'<span class="hz-figure">' +
							propertyValues.get(`${map_properties[i].property_id}-area`) +
							'</span>' +
							'</li>';

						innerHTML +=
							'<!-- TheGrimSilence: Retrieve Global Property Values -->';
						innerHTML += '</ul>';
			...
			var price_range_search = function (min_price, max_price) {
				...
				if (currency_position == 'after') {
					var min_price_range =
						addCommas($('.price-range').slider('values', 0)) + currency_symb;
					var max_price_range =
						addCommas($('.price-range').slider('values', 1)) + currency_symb;
				} else {
					var min_price_range =
						currency_symb + addCommas($('.price-range').slider('values', 0));
					var max_price_range =
						currency_symb + addCommas($('.price-range').slider('values', 1));
				}
