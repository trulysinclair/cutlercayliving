...
if(!function_exists('houzez_enqueue_osm_location_js')) {
    function houzez_enqueue_osm_location_js() {
        ...
        if( !wp_script_is('houzez-osm-properties') ) {
            $map_options = array();
            wp_register_script( 'houzez-osm-properties', HOUZEZ_JS_DIR_URI . 'osm-properties' . houzez_minify_js() . '.js', array( 'jquery', 'leaflet' ), HOUZEZ_THEME_VERSION, true );
...
if( !function_exists( 'houzez_get_single_property_osm_map' ) ) {
    ...
    function houzez_get_single_property_osm_map() {
        ...
        wp_register_script( 'houzez-single-property-map', HOUZEZ_JS_DIR_URI . 'single-property-osm-map' . houzez_minify_js() . '.js', array( 'jquery', 'leaflet' ), HOUZEZ_THEME_VERSION, true );
        ...
        if( !empty($location) && $show_map != 0 && !empty($address) ) {
